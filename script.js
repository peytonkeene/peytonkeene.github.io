document.addEventListener("DOMContentLoaded", function() {
    ...
    // Update refuser text in refusal narrative
    refuser.addEventListener("change", function() {
        const selectedRefuser = refuser.options[refuser.selectedIndex].text;
        for (let i = 1; i <= 7; i++) {
            document.getElementById(`refuserSpan${i}`).innerText = selectedRefuser;
        }
    });
...
function generateNarrative() {
    let narrative = createDispatchNarrative() +
                    ...
                    (document.getElementById("painCheck").checked ? createPainNarrative() : '') +
                    createHistoryNarrative() +
                    createAssessmentNarrative() +
                    createTreatmentNarrative() +
                    (document.getElementById("transportToggle").checked ? createTransportNarrative() : '') +
                    (document.getElementById("refusalToggle").checked ? createRefusalNarrative() : '') +
                    createSmartDocsNarrative();
...
function createRefusalNarrative() {
    const selectedRefuser = document.getElementById("refuser").options[document.getElementById("refuser").selectedIndex].text;
    return `Refusal: The ${selectedRefuser} has refused to be transported to the hospital for further evaluation and care. The ${selectedRefuser} is oriented, clear of mind, and has the capacity to understand the presented information. The ${selectedRefuser} has verbalized full understanding of their symptoms and understands that forgoing further evaluation and/or treatment could pose a significant medical risk to the patient's life. The ${selectedRefuser} has verbalized that they understand our treatment plan, including interventions and transport destinations, and does not want these interventions currently. Furthermore, the ${selectedRefuser} acknowledges that forgoing this treatment could lead to a worsening of the condition up to and including death. The ${selectedRefuser} understands that they are free to call 911 should the condition worsen, or they later decide that they wish to be transported to the Emergency Department for further evaluation and intervention. The ${selectedRefuser} acknowledged and assumed risks and signed the EMS Refusal Form.\n\n`;
}
