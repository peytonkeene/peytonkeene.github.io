...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If no errors, update client in the database
    if (empty($nameError) && empty($emailError)) {
        $stmt = $conn->prepare("UPDATE clients SET name = ?, email = ?, phone = ?, status = ? WHERE id = ?");
        $stmt->bind_param("ssssi", $clientName, $clientEmail, $clientPhone, $clientStatus, $clientId);
...
<head>
    ...
    <title>Edit Client - MedNarrate CRM</title>
...
<body>
    ...
    <main>
        <div class="content">
            ...
            <?php endif; ?>
            <form action="edit_client.php?id=<?= $clientId ?>" method="POST">
