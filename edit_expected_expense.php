...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($budget_type == 'monthly') {
        ...
        $sql = "UPDATE expected_expenses SET description=?, amount=?, date=?, year=?, month=? WHERE expected_expense_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sdsiii", $description, $amount, $date, $year, $month, $id);
    } else {
        $year = date('Y', strtotime($date));
        $sql = "UPDATE expected_expenses SET description=?, amount=?, date=?, year=? WHERE expected_expense_id=?";
        $stmt = $conn->prepare($sql);
    ...
    $sql = "SELECT * FROM expected_expenses WHERE expected_expense_id=?";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Edit Expected Expense - MedNarrate</title>
    ...
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <select id="budget_type" name="budget_type" required>
                <option value="monthly" <?php echo ($expense['month'] ? 'selected' : ''); ?>>Monthly</option>
                <option value="yearly" <?php echo (!$expense['month'] ? 'selected' : ''); ?>>Yearly</option>
            </select>

            <button type="submit" class="btn">Update Expected Expense</button>
        </form>
    </div>
</body>
</html>
