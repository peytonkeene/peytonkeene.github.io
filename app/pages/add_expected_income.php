...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($budget_type == 'monthly') {
        ...
        $sql = "INSERT INTO expected_incomes (description, amount, date, year, month) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sdsii", $description, $amount, $date, $year, $month);
    } else {
        $year = date('Y', strtotime($date));
        $sql = "INSERT INTO expected_incomes (description, amount, date, year) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Add Expected Income - MedNarrate</title>
...
<body>
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn">Add Expected Income</button>
        </form>
    </div>
</body>
</html>
