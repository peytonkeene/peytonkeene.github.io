...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Prepare the SQL statement to update the income
    $sql = "UPDATE incomes SET description = ?, date = ?, amount = ? WHERE income_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssdi', $description, $date, $amount, $income_id);

    if ($stmt->execute()) {
        header("Location: interactive_budget.php?success=Income updated successfully");
        exit();
    } else {
    ...
    // Fetch the existing income data for pre-filling the form
...
<head>
    ...
    <title>Edit Income - MedNarrate</title>
    <link rel="icon" href="images/favicon.ico" type="image/x-icon"> <!-- Favicon -->
    <style>
        /* Include the same styles as the other pages to keep the design consistent */
        body {
...
<body>
    <header>
        ...
        <nav>
            <a href="interactive_budget.php">Back to Budget Manager</a>
    ...
    <main>
        <div class="content">
            <h1>Edit Income</h1>
            <form action="edit_income.php?id=<?php echo htmlspecialchars($income_id); ?>" method="POST">
